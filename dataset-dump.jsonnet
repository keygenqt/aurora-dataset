{
  dataset: {
    faq: {
      ...
      ru: [
        {
          t: 'Демонстрационный_ответ_с_набором_всех_элементов.',
          b: |||
            Это демонстрация возможного ответа.
            Для проверки работоспособности бота его понимать.
            Для демонстрации всех возможностей MarkdownV2 который доступен по ссылке:
            https://core.telegram.org/bots/api#markdownv2-style
            или такой ссылке: [MarkdownV2](https://core.telegram.org/bots/api#markdownv2-style)
            
            1. Мы можем так: *bold text*
            2. Мы можем так: _italic text_
            3. Мы можем так: __underline__
            4. Мы можем так: ~strikethrough~
            5. Мы можем так: `inline fixed-width code`
            
            Мы можем добавлять код:
            ```cpp
        ...
        },
        {
          ...
          b: |||
            Это интерфейс командной строки Flutter, который обеспечивает основную работу с фреймворком. Устанавливается вместе с Flutter Framework.
        ...
        },
        {
          ...
          b: 'Flutter Engine — это переносимая среда выполнения для хостинга приложений Flutter. Она реализует основные библиотеки Flutter, включая анимацию и графику, файловый и сетевой ввод-вывод. Поддержку доступности, архитектуру плагинов, среду выполнения Dart и цепочку инструментов компиляции. Большинство разработчиков будут взаимодействовать с Flutter через Flutter Framework.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        {
          t: 'Что такое SDK?',
          b: 'SDK (Software Development Kit) - набор инструментов для разработки программного обеспечения объединённый в одном пакете. Flutter - SDK по сути, framework он содержит в SDK.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        },
        {
          ...
          b: |||
            ...
            [https://omprussia.gitlab.io/flutter/flutter/support](https://omprussia.gitlab.io/flutter/flutter/support)
          |||,
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
        ...
        },
        {
          ...
          b: |||
            ...
            [https://gitlab.com/omprussia/flutter](https://gitlab.com/omprussia/flutter)
          |||,
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
        ...
        },
        {
          ...
          b: |||
            ...
            [https://omprussia.gitlab.io/flutter/flutter](https://omprussia.gitlab.io/flutter/flutter)
          |||,
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
        ...
        },
        {
          ...
          b: 'Да, бывают плагины не использующие С++ код. Такой плагин можно написать с использованием другого плагина - [https://pub.dev/packages/dbus](https://pub.dev/packages/dbus). На выходе вы получите пакет, который использует плагин в зависимостях.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        },
        {
          ...
          b: 'На данный момент поддержки ОС Аврора в целом нет. Но работы в этом направлении ведутся.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        },
        {
          ...
          b: 'На данный момент Flutter для ОС Аврора поддерживает только Linux (рекомендована Ubuntu). WSL2 на Windows работает лишь частично, полного функционала Flutter CLI нет.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        },
        {
          ...
          b: |||
            Выполнить можно это использую Flutter CLI. Для этого перейдите в проект и обновите зависимости:
            ```shell
        ...
        },
        {
          ...
          b: |||
            Официально такого образа нет. Есть пет проект который работает на Linux, с флагом `--privileged`:
            [https://github.com/keygenqt/flutter-aurora-docker](https://github.com/keygenqt/flutter-aurora-docker)
          |||,
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
        ...
        },
        {
          ...
          b: 'Есть разница при запуске приложения из IDE VS Code и запуском приложения через иконку. Если IDE запускает из под пользователя, то запуск через иконку происходит в изолированном окружении. В изоляции (в песочнице) Dart debug работать не будет - окружение не дает запустить сервер DevTools.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        },
        {
          ...
          b: 'Вы можете конфигурировать Flutter без необходимости использовать `flutter-aurora config`. Файлы конфигурации храняться в формате json, главное знать что менять. Их можно найти в директории: `~/.config/flutter`.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        },
        {
          ...
          b: |||
            ...
            flutter-aurora doctor
            ```
            Она найдет некоторые возможные ошибки, но основательно можно это проверить - собрать демо проект [Fluttery ToDo](https://gitlab.com/omprussia/flutter/fluttery-todo).
          |||,
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
        ...
        },
        {
          ...
          b: |||
            Изначально Flutter для ОС Аврора поддерживал только Linux (рекомендовано Ubuntu). Более детально уточнить этот вопрос можно в документации проекта:
            [https://omprussia.gitlab.io/flutter/flutter](https://omprussia.gitlab.io/flutter/flutter)
          |||,
          a: {
            fname: 'Vitaliy',
        ...
        },
        {
          ...
          b: 'Для форматирования проектов необходим Dart. Flutter SDK содержит его копию и мы можем без установки отдельной версии Dart форматировать проекты. Для этого можно добавить алиас, Dart можно найти `<flutter_dir>/bin/dart`.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        },
        {
          ...
          b: 'Возможно причина в том что приложение с иконке запускается в песочнице, замещенном окружении. Необходимо проверить разрешения (permission) приложения, это часто является причиной такого поведения.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        },
        {
          ...
          b: |||
            Вы можете выполнить это средствами IDE (VS Code). Или выполнив команду в терминале указав пусть к файлу с тестами:
            ```shell
        ...
        },
        {
          ...
          b: |||
            Пакет должен быть подписан. ОС Аврора имеет валидацию пакетов при их установке. Необходимо провалидировать пакет с помощью aurora-cli или psdk. Если с этим порядок более детальную информацию можно получить подключившись по ssh к девайсу или эмулятору. Выполнить команду перед установкой и посмотреть логи:
            ```shell
            journalctl -u apm -f
            ```
          |||,
        ...
        },
        {
          ...
          b: 'Нет, IDEA не поддерживается. Но VS Code работает отлично, а так как предстоит, возможно, работа с С++ это IDE отлично подойдет.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        },
        {
          ...
          b: |||
            Qt без сигналов и слотов работает без дополнительных манипуляций. Если нужны именно сигналы Qt необходимо активировать их работу.
        ...
        },
        {
          ...
          b: |||
            ...
            [https://developer.auroraos.ru/doc/sdk/tools/rpmsign_external](https://developer.auroraos.ru/doc/sdk/tools/rpmsign_external)
            Aurora CLI умеет тоже подписывать пакеты:
            [https://keygenqt.github.io/aurora-cli/cli/psdk/package/sign](https://keygenqt.github.io/aurora-cli/cli/psdk/package/sign)
          |||,
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
        ...
        },
        {
          ...
          b: |||
            Это окружение, в котором запускается приложение, ограниченное в доступе к системе. Из терминала запустить приложение в этом окружении можно командой:
            ```shell
        ...
        },
        {
          ...
          b: 'Open-source приложение обеспечивающее легкий старт в экосистеме ОС Aurora. Работа с Aurora SDK, Aurora Platform SDK, Flutter SDK, взаимодействие с устройствами и эмулятором, а также многое другое.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        },
        {
          ...
          b: 'Для этого нужно посмотреть на `pubspec.yaml`. Если в спеке нет раздела `plugin` и нет зависимостей от плагинов - это пакет.',
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
          },
        ...
        },
        {
          ...
          b: |||
            ...
            [https://github.com/flutter/engine/blob/main/shell/platform/common/client_wrapper/README](https://github.com/flutter/engine/blob/main/shell/platform/common/client_wrapper/README)
          |||,
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
        ...
        },
        {
          ...
          b: |||
            ...
            [https://gitlab.com/omprussia/flutter/flutter/-/tags](https://gitlab.com/omprussia/flutter/flutter/-/tags)
          |||,
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
        ...
        },
        {
          ...
          b: |||
            Проект Flutter для ОС Аврора - открытый проект. Вы можете помощь при разработке как проекта так и его документации. Документация проекта использует mkdocs & mkdocs-material, это Python пакеты.
        ...
        },
        {
          ...
          b: 'В Аврора 5 стала обязательной дополнительная подпись на RPM пакете - подпись источника. Подпись источника криптографически заверяет того, кем был предоставлен тот или иной RPM пакет. Зачастую подпись источника добавляется официальными маркетами, например RuStore, когда вы загружаете свое приложение через админ панель. Для целей разработки вы можете отключить обязательное требование подписи источника пакетов через настройки.',
          a: {
            fname: 'Denis',
            lname: 'Glazkov',
          },
        ...
        },
        {
          ...
          b: |||
            ...
            [https://keygenqt.github.io/aurora-cli/install](https://keygenqt.github.io/aurora-cli/install)
          |||,
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
        ...
        },
        {
          ...
          b: |||
            ...
            [https://github.com/keygenqt/aurora-toolbox/releases](https://github.com/keygenqt/aurora-toolbox/releases)
            Установка отличается на дистрибутивах Linux, посмотрите документацию вашего пакетного менеджера.
            Более подробно написано на странице документации приложения:
            [https://keygenqt.github.io/aurora-toolbox/install](https://keygenqt.github.io/aurora-toolbox/install)
          |||,
          a: {
            fname: 'Vitaliy',
            lname: 'Zarubin',
